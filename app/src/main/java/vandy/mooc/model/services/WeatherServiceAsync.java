        ...
        new WeatherRequest.Stub() {
                                          ...
                                          final WeatherResults callback) throws RemoteException
            {
                ...
                final Runnable getCurrentWeatherRunnable = new Runnable() {
                    public void run() {
                        try {
                            final List<WeatherData> weatherDataList = getWeatherResults(location);

                            if (weatherDataList != null) {
                                Log.d(TAG, ""
                                        + weatherDataList.size()
                                        + " result(s) for Weather Location: "
                                        + location);

                                for (WeatherData weatherData : weatherDataList) {
                                    try {
                                        callback.sendResults(weatherData);
                                ...
                                try {
                                    callback.sendError("No Weather Data found for \""
                                            + location
                                            + "\" location");
                                } catch (RemoteException e) {
                                    e.printStackTrace();
